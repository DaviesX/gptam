...
void
Tracker::TrackFrame(cv::Mat_<uchar> &imFrame, bool bDraw, cv::Mat &rgbFrame)
{
        mbDraw = bDraw;
        ...
        if(mbDraw) {
                ...
                if(Persistence::PV3::get<int>("Tracker.DrawFASTCorners",1, Persistence::SILENT)) {
...
cv::Vec<float, 6> Tracker::CalcPoseUpdate(vector<TrackerData::Ptr> vTD, double dOverrideSigma, bool bMarkOutliers) {
        ...
        int nEstimator = 0;
        static Persistence::pvar3<string> pvsEstimator("TrackerMEstimator", "Tukey", Persistence::SILENT);
